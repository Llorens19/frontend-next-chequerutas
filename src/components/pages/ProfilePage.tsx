...
const ProfileUser = ({ username }: { username: string }) => {
  ...
  const { data: userLogged, isLoading: isLoadingUserLogged } =
    useGetUserQuery();
  const { data: routesPublic, isLoading: isLoadingRoutesUserPublic } =
    useRoutesUserPublic(username);
  const { data: routesPrivate, isLoading: isLoadingRoutesUserPrivate } =
    useRoutesUserPrivate(username);
      ...
      {isMobile ? (
        <div className="flex flex-col w-full p-4 pt-8 pb-20">
          <div className="flex w-full">
            ...
            <div className="flex flex-col w-3/4 px-4 justify-center align-middle">
              <h1 className="text-2xl font-bold mt-2 text-text1 mx-auto">
                {user.username}
              </h1>
        ...
        <>
          ...
          <div className="mt-36 w-4/5 mx-auto flex">
            <div className="flex flex-col self-start items-center w-1/5 bg-color2 rounded-3xl py-8 fixed  ">
              ...
              <h1 className="text-2xl font-bold mt-2 text-text1">
                {user.username}
              </h1>
              <p className="text-lg text-text3">{user.email}</p>
