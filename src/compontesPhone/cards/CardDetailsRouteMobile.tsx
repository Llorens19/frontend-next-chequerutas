...
import { useDeleteRouteMutation } from '@/reactQuery/mutations/route.mutations';

const CardDetailRouteMobile = ({ route }: ICardDetailRouteParams) => {
  const router = useRouter();

  const delteRoute = useDeleteRouteMutation();
  ...
  const onClickDelete = () => {
    delteRoute.mutateAsync(route.idRoute);
    router.push('/');
    ...
    <>
      <div className="flex flex-col p-4 m-auto rounded-3xl gap-4 mb-20">
        ...
        <div className="flex justify-between bg-color2 rounded-3xl p-4 max-h-20">
          <div
            className="flex  bg-color4 rounded-full justify-center items-center  cursor-pointer"
          ...
          {userLogged && userLogged.username === route.user?.username && (
            <div
              className="flex items-center cursor-pointer text-text1"
            ...
            >
              <svg
                ...
                // className="lucide lucide-trash w-6 h-6 mr-2"
              >
